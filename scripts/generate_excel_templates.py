...
def build_vehicles():
    ...
    headers = ["מספר רכב", "מקצוע", "מחלקה"]
    ws.append(headers)
    style_header_row(ws)

    sample_rows = [
        ["12-345-67", "רכב",     "מחלקת תחזוקה"],
        ["23-456-78", "חשמל",    "מחלקת חשמל"],
        ["34-567-89", "רכב",     ""],
        ["45-678-90", "אופטיקה", "מחלקת אופטיקה"],
        ["56-789-01", "חשמל",    "מחלקת חשמל"],
    ]
    for row in sample_rows:
        ws.append(row)
    for r in range(2, 2 + len(sample_rows)):
    ...
    autosize_columns(ws, {"A": 16, "B": 16, "C": 22})
    ...
    add_instructions_sheet(wb, [
        ...
        "",
        "עמודות:",
        "  • מספר רכב — מספר/מזהה ייחודי. דוגמה: \"12-345-67\" (מספר רישוי) או \"G500-001\" (מזהה פנימי).",
        "  • מקצוע — שם המקצוע שאחראי על הרכב. חייב להתאים לאחד הערכים שיופיעו בעמודת \"מקצוע\" ב-employees.xlsx.",
